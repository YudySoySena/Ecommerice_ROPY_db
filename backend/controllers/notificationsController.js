...
const createNotification = (req, res) => {
  ...
  req.getConnection((err, conn) => {
    ...
    const query =
      "INSERT INTO Notifications (userId, title, message, date, status) VALUES (?, ?, ?, ?, ?)";
...
const updateNotificationStatus = (req, res) => {
  ...
  req.getConnection((err, conn) => {
    ...
    const query =
      "UPDATE Notifications SET status = ? WHERE notificationId = ?";
    conn.query(query, [status, id], (err, result) => {
      if (err) {
        console.error(err);
        return res
          .status(500)
          .send("Error al actualizar el estado de la notificación.");
      }
      res.json({
